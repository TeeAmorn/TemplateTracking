    ...
    for filename in sorted(os.listdir(images_dir)):
        if filename.endswith(".jpg"):
            ...
            if (isFirst):
                ...
                window_size = first_template.shape[0]+8

                # If window goes beyond top-left corner
                if ((curr_pos[0]-window_size//2 < 0) and (curr_pos[1]-window_size//2 < 0)):
                    search_wnd = frame[0:window_size,
                                       0:window_size]
                    tpl_center = (window_size//2, window_size//2)
                    curr_pos = tpl_center

                # If window goes beyond top-right corner
                elif ((curr_pos[0]-window_size//2 < 0) and (curr_pos[1]+window_size//2 >= frame_width)):
                    search_wnd = frame[0:window_size,
                                       -window_size:]
                    tpl_center = (window_size//2, window_size//2)
                    curr_pos = (window_size//2, frame_width-window_size//2-1)
                
                # If window goes beyond bottom-left corner
                elif ((curr_pos[0]+window_size//2 >= frame_height) and (curr_pos[1]-window_size//2 < 0)):
                    search_wnd = frame[-window_size:,
                                       0:window_size]
                    tpl_center = (window_size//2, window_size//2)
                    curr_pos = (frame_height-window_size//2-1, window_size//2)
                
                # If window goes beyond bottom-right corner
                elif ((curr_pos[0]+window_size//2 >= frame_height) and (curr_pos[1]+window_size//2 >= frame_width)):
                    search_wnd = frame[-window_size:,
                                       -window_size:]
                    tpl_center = (window_size//2, window_size//2)
                    curr_pos = (frame_height-window_size//2-1, frame_width-window_size//2-1)

                # If window goes beyond left edge
                elif (curr_pos[1]-window_size//2 < 0):
                    search_wnd = frame[(curr_pos[0]-window_size//2):(curr_pos[0]+window_size//2+1),
                                       0:window_size]
                    tpl_center = (window_size//2, window_size//2)
                    curr_pos = (curr_pos[0], window_size//2)

                # If window goes beyond top edge
                elif (curr_pos[0]-window_size//2 < 0):
                    search_wnd = frame[0:window_size,
                                       (curr_pos[1]-window_size//2):(curr_pos[1]+window_size//2+1)]
                    tpl_center = (window_size//2, window_size//2)
                    curr_pos = (window_size//2, curr_pos[1])
                
                # If window goes beyond right edge
                elif (curr_pos[1]+window_size//2 >= frame_width):
                    search_wnd = frame[(curr_pos[0]-window_size//2):(curr_pos[0]+window_size//2+1),
                                       -window_size:]
                    tpl_center = (window_size//2, window_size//2)
                    curr_pos = (curr_pos[0], frame_width-window_size//2-1)
                
                # If window goes beyond bottom edge
                elif (curr_pos[0]+window_size//2 >= frame_height):
                    search_wnd = frame[-window_size:,
                                       (curr_pos[1]-window_size//2):(curr_pos[1]+window_size//2+1)]
                    tpl_center = (window_size//2, window_size//2)
                    curr_pos = (frame_height-window_size//2-1, curr_pos[1])
                ...
                prev_tpl, dydx = search(search_wnd, prev_tpl, tpl_center, match_method)
                curr_pos = (curr_pos[0]+dydx[0], curr_pos[1]+dydx[1])
                
            tracked_frame = draw_box(frame_colored, curr_pos, 41, 41)
            cv.imwrite(os.path.join("draft2_ncc_results_43", filename), tracked_frame)
...
track("video", tpl, (43, 71), ncc) # 43x43